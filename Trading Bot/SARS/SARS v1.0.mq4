...
void OnTick()
{
   ...
   {
      bool enterB = false;
      double c1B = iClose(_Symbol,_Period,1);
      double c2B = iClose(_Symbol,_Period,2);
      double ema20B = iMA(_Symbol,_Period,20,0,MODE_EMA,PRICE_CLOSE,0);
      double ema50B = iMA(_Symbol,_Period,50,0,MODE_EMA,PRICE_CLOSE,0);
      if(posSwitch() == 2)
      {
         if(c1B < ema20B && c1B < ema50B)
            enter = true;
      }
      else if(posSwitch() == 3)
      {
         if((c1B < ema20B || c2B < ema20B) && (c1B < ema50B || c2B < ema50B))
            enter = true;
      }
      double entB = Ask-10*Point;
      double stopB = iHigh(_Symbol,_Period,0)+10*Point;
      double profitB = entB - (stopB - entB)*1.8;
      if(enterB == true && OrdersTotal() == 0)
         double ticketB = OrderSend(_Symbol,OP_SELLSTOP,0.01,entB,2,stopB,profitB,NULL,0,0,Green); 
   }
