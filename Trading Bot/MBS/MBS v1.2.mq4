...
bool segment()
{
   ...
   if(m1>c1)
   {
      for(i = 1;; i++)
      {
        if(iBands(_Symbol, _Period, 20,2,0, PRICE_CLOSE, MODE_MAIN,i)<iClose(_Symbol,_Period,i))
         break;
      }
   }
   ...
   if(m1<c1)
   {
      for(k = 1;; k++)
      {
        if(iBands(_Symbol, _Period, 20,2,0, PRICE_CLOSE, MODE_MAIN,k)>iClose(_Symbol,_Period,k))
         break;
      }
   }   
   if(OrderSelect(OrdersTotal()-1,SELECT_BY_POS) == true)
   {
      ...
      long SwitchTime = iTime(_Symbol,_Period,k-1);
