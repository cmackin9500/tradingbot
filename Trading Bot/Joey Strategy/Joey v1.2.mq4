...
void OnTick()
{
   if(OrdersTotal() == 0 && oneque() == true && fiveque() == true && otherque() == true)
   {
      ...
      for(int i = 3; i > 0; i--)
      {
         if(down > iLow(_Symbol,_Period,i))
         {
            down = iLow(_Symbol,_Period,i);
         }
      }
      down = Ask - down;
      if(up > down)
    ...
    //  double enter = Ask + _Point*5;
      if(up/down > 0.6 && Ask < profit)
      {
         int ticket = OrderSend(_Symbol,OP_BUY,0.01,Ask,3,stoploss,profit,NULL,0,0,Green);  
      }
      else
      {
         profit = Ask + down*0.6;
